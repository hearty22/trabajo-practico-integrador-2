...
export const ownerOrAdminComment = async (req,res,next) => {
    try {
        const token = req.cookies.token;
        if(!token){return res.status(401).json({ok: false, msg:"no token provided"});};
        const decoded = verifyToken(req);
        if(decoded.role != "admin"){
            ...
            if(!comment){return res.status(404).jon({ok: false, msg:"article not found"})}
            if(toString(comment.author)== decoded.id){
                next();
            };
            return res.status(403).json({ok: false, msg:"acces denied: no owner"})
        };
